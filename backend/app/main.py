...
app = FastAPI(title="IDS Manager API", version="0.2.0")
...
def criar_empresa(payload: schemas.EmpresaCreate, db: Session = Depends(get_db)):
    ...
    emp = models.Empresa(id_empresa=novo_id, nome=payload.nome)
...
def excluir_empresa(empresa_pk: int, db: Session = Depends(get_db)):
    ...
    db.delete(emp); db.commit(); return
...
def criar_unidade(payload: schemas.UnidadeCreate, db: Session = Depends(get_db)):
    ...
    novo_id = next_id_unidade(db, payload.empresa_id)
    und = models.Unidade(id_unidade=novo_id, nome=payload.nome, empresa_id=payload.empresa_id)
    db.add(und); db.commit(); db.refresh(und); return und

@app.get("/unidades", response_model=list[schemas.UnidadeOut])
def listar_unidades(empresa_id: int | None = Query(None), db: Session = Depends(get_db)):
...
def criar_item(payload: schemas.ItemCreate, db: Session = Depends(get_db)):
    ...
    if existe: raise HTTPException(409, "Já existe item com esse id_item nessa unidade")
    item = models.Item(
        id_item=id_item, tipo=payload.tipo, ano_mes=payload.ano_mes,
        titulo_visivel=payload.titulo_visivel, caminho_arquivo=payload.caminho_arquivo,
...
def listar_itens(
    ...
    if unidade_id is not None: query = query.filter(models.Item.unidade_id == unidade_id)
    if empresa_id is not None: query = query.join(models.Unidade).filter(models.Unidade.empresa_id == empresa_id)
    if tipo is not None: query = query.filter(models.Item.tipo == tipo)
    if ano_mes is not None: query = query.filter(models.Item.ano_mes == ano_mes)
    if q: query = query.filter(models.Item.titulo_visivel.ilike(f"%{q}%"))
...
def validar_nome(payload: dict):
    ...
    if not nome: raise HTTPException(400, "Informe 'nome_arquivo'")
...
def montar_pastas_unidade(unidade_pk: int, payload: dict, db: Session = Depends(get_db)):
    ...
    if not base_dir: raise HTTPException(400, "Informe 'base_dir'")
    und = db.get(models.Unidade, unidade_pk)
    if not und: raise HTTPException(404, "Unidade não encontrada")
